...
<body>
 ...
 <div class="container">
  <div class="row">
   <h1 class="text-center">Hello This is post</h1>
   <!-- Blog entries-->
   <div class="col-lg-8">

    <?php
   include("config/db_con.php");
    $query = $link->query("SELECT posts.*, users.username AS name, categories.categories_name AS cat_name FROM posts LEFT JOIN users ON posts.user_id = users.id LEFT JOIN categories ON posts.category_id = categories.id");
   ...
   $query = $link->query("SELECT posts.*, users.username AS name, categories.categories_name AS cat_name FROM posts LEFT JOIN users ON posts.user_id = users.id LEFT JOIN categories ON posts.category_id = categories.id ORDER BY posts.created_at DESC LIMIT $offset, $num_per_page");
   ?>
    <!-- Featured blog post-->
